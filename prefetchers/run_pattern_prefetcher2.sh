...
gcc -Wall -o dpc2sim2 $TEST_PREFETCHER_LOCATION ../lib/dpc2sim.a

# each configuration setting 
while read currentLine;
  do 
  # Pipe current line into the test_line2.csv file
  rm test_line2.csv
  echo $currentLine > test_line2.csv
  ...
  for trace in $TRACES
    ...
    echo -ne $traceName | sed -e "\$a,"  >> currentTrace2
    echo -ne $TEST_PREFETCHER_NAME | sed -e "\$a," >> currentTrace2

    echo "Flag: NONE"
    # Each line: run executable, fetch last number, append, append to current file 
    cat $trace | ./dpc2sim2 | awk '{w=NF?$NF:w} END{print w}' |  sed -e "\$a," >> currentTrace2

    echo  "Flag: small_llc"
    cat  $trace | ./dpc2sim2  -small_llc | awk '{w=NF?$NF:w} END{print w}' | sed -e "\$a,">> currentTrace2

    echo "Flag: low_bandwidth"
    cat  $trace | ./dpc2sim2  -low_bandwidth | awk '{w=NF?$NF:w} END{print w}'| sed -e "\$a," >> currentTrace2

    echo  "Flag: scramble_loads"
    cat $trace | ./dpc2sim2  -scramble_loads | awk '{w=NF?$NF:w} END{print w}'| sed -e "\$a," >> currentTrace2

    # Calculate the sum
    # Takes the currentTrace2 file, prints even lines (to get rid of commas)
    # removes top line (name of the trace)
    # sums up all of the numbers 
    # adds comma!
    cat currentTrace2 | awk 'NR % 2' | tail -n +2  | awk '{s+=$1} END {print s}' | sed -e "\$a," >> currentTrace2


    # Write the current settings to the csv file 
    echo -ne $THRESHOLD | sed -e "\$a," >> currentTrace2
    echo -ne $LOW_DEGREE | sed -e "\$a," >> currentTrace2
    echo -ne $STANDARD_DEGREE | sed -e "\$a," >> currentTrace2
    # No comma on the last line
    echo -ne $HIGH_DEGREE >> currentTrace2


    # Clean up new lines in current trace run, append to end of file for the output file 
    # For each extra newline in currentTrace2, increment NR%#?
    #Number of columns + 8
    awk '{ ORS = (NR%21? "" : RS) } 1' currentTrace2 >> $NEWFILE
    rm currentTrace2
    done 
done <$CONFIG_FILE
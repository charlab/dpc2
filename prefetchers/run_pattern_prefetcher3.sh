...
gcc -Wall -o dpc2sim3 $TEST_PREFETCHER_LOCATION ../lib/dpc2sim.a

# each configuration setting 
while read currentLine;
  do 
  # Pipe current line into the test_line3.csv file
  rm test_line3.csv
  echo $currentLine > test_line3.csv
  ...
  for trace in $TRACES
    ...
    echo -ne $traceName | sed -e "\$a,"  >> currentTrace3
    echo -ne $TEST_PREFETCHER_NAME | sed -e "\$a," >> currentTrace3

    echo "Flag: NONE"
    # Each line: run executable, fetch last number, append, append to current file 
    cat $trace | ./dpc2sim3 | awk '{w=NF?$NF:w} END{print w}' |  sed -e "\$a," >> currentTrace3

    echo  "Flag: small_llc"
    cat  $trace | ./dpc2sim3  -small_llc | awk '{w=NF?$NF:w} END{print w}' | sed -e "\$a,">> currentTrace3

    echo "Flag: low_bandwidth"
    cat  $trace | ./dpc2sim3  -low_bandwidth | awk '{w=NF?$NF:w} END{print w}'| sed -e "\$a," >> currentTrace3

    echo  "Flag: scramble_loads"
    cat $trace | ./dpc2sim3  -scramble_loads | awk '{w=NF?$NF:w} END{print w}'| sed -e "\$a," >> currentTrace3

    # Calculate the sum
    # Takes the currentTrace3 file, prints even lines (to get rid of commas)
    # removes top line (name of the trace)
    # sums up all of the numbers 
    # adds comma!
    cat currentTrace3 | awk 'NR % 2' | tail -n +2  | awk '{s+=$1} END {print s}' | sed -e "\$a," >> currentTrace3


    # Write the current settings to the csv file 
    echo -ne $THRESHOLD | sed -e "\$a," >> currentTrace3
    echo -ne $LOW_DEGREE | sed -e "\$a," >> currentTrace3
    echo -ne $STANDARD_DEGREE | sed -e "\$a," >> currentTrace3
    # No comma on the last line
    echo -ne $HIGH_DEGREE >> currentTrace3


    # Clean up new lines in current trace run, append to end of file for the output file 
    # For each extra newline in currentTrace3, increment NR%#?
    #Number of columns + 8
    awk '{ ORS = (NR%21? "" : RS) } 1' currentTrace3 >> $NEWFILE
    rm currentTrace3
    done 
done <$CONFIG_FILE
...
gcc -Wall -o dpc2sim5 $TEST_PREFETCHER_LOCATION ../lib/dpc2sim.a

# each configuration setting 
while read currentLine;
  do 
  # Pipe current line into the test_line5.csv file
  rm test_line5.csv
  echo $currentLine > test_line5.csv
  ...
  for trace in $TRACES
    ...
    echo -ne $traceName | sed -e "\$a,"  >> currentTrace5
    echo -ne $TEST_PREFETCHER_NAME | sed -e "\$a," >> currentTrace5

    echo "Flag: NONE"
    # Each line: run executable, fetch last number, append, append to current file 
    cat $trace | ./dpc2sim5 | awk '{w=NF?$NF:w} END{print w}' |  sed -e "\$a," >> currentTrace5

    echo  "Flag: small_llc"
    cat  $trace | ./dpc2sim5  -small_llc | awk '{w=NF?$NF:w} END{print w}' | sed -e "\$a,">> currentTrace5

    echo "Flag: low_bandwidth"
    cat  $trace | ./dpc2sim5  -low_bandwidth | awk '{w=NF?$NF:w} END{print w}'| sed -e "\$a," >> currentTrace5

    echo  "Flag: scramble_loads"
    cat $trace | ./dpc2sim5  -scramble_loads | awk '{w=NF?$NF:w} END{print w}'| sed -e "\$a," >> currentTrace5

    # Calculate the sum
    # Takes the currentTrace5 file, prints even lines (to get rid of commas)
    # removes top line (name of the trace)
    # sums up all of the numbers 
    # adds comma!
    cat currentTrace5 | awk 'NR % 2' | tail -n +2  | awk '{s+=$1} END {print s}' | sed -e "\$a," >> currentTrace5


    # Write the current settings to the csv file 
    echo -ne $THRESHOLD | sed -e "\$a," >> currentTrace5
    echo -ne $LOW_DEGREE | sed -e "\$a," >> currentTrace5
    echo -ne $STANDARD_DEGREE | sed -e "\$a," >> currentTrace5
    # No comma on the last line
    echo -ne $HIGH_DEGREE >> currentTrace5


    # Clean up new lines in current trace run, append to end of file for the output file 
    # For each extra newline in currentTrace5, increment NR%#?
    #Number of columns + 8
    awk '{ ORS = (NR%21? "" : RS) } 1' currentTrace5 >> $NEWFILE
    rm currentTrace5
    done 
done <$CONFIG_FILE